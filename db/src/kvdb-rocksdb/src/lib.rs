...
use std::{cmp, error, fs, io, path::Path, result};

use rocksdb::{
    BlockBasedOptions, ColumnFamily, ColumnFamilyDescriptor, DBIterator,
    Direction, Error, IteratorMode, Options, ReadOptions, WriteBatch,
    WriteOptions, DB,
};

use kvdb::{
    DBValue, IterationHandler, MigrationHandler, NumColumns, OpenHandler,
    ReadTransaction, TransactionHandler, WriteTransaction,
};
...
const CF_HANDLE_PROOF: &'static str =
...
impl DBAndColumns {
    fn options(config: &DatabaseConfig) -> Options {
        ...
        if let Some(_rate_limit) = config.compaction.write_rate_limit {
            // TODO: add to upstream
            // opts.set_parsed_options(&format!("rate_limiter_bytes_per_sec={}",
            // rate_limit)).map_err(other_io_err)?;
        }
...
impl OpenHandler<DBAndColumns> for DBAndColumns {
    ...
    fn open(config: &Self::Config, path: &str) -> io::Result<DBAndColumns> {
...

        {
            ...
            let cache_size =
                cmp::max(8 * 1024 * 1024, config.memory_budget() / 3);
            ...
            Err(ref s) if is_corrupted(&s.clone().into_string()) => {
                ...
                match cfnames.is_empty() {
                    ...
                    false => {
                        ...
                        let cf_descriptors: Vec<_> = cfnames
                            .iter()
                            .zip(cf_options)
                            .map(|(name, option)| {
...
impl<'a> WriteTransaction for RocksDBWriteTransaction<'a> {
    ...
    fn commit(self: Box<Self>) -> io::Result<()> {
        ...
        check_for_corruption(path, db.write_opt(batch, write_opts))
    }
}
...
impl DBAndColumns {
    fn column_families(&self) -> Vec<ColumnFamily> {
        self.cf_names
            .iter()
            .map(|name| self.db.cf_handle(&name).expect(CF_HANDLE_PROOF))
            .collect()
    }
}
...
impl MigrationHandler<DBAndColumns> for DBAndColumns {
    ...
    fn add_column(
        &mut self, config: &<DBAndColumns as OpenHandler<DBAndColumns>>::Config,
    ) -> io::Result<()> {
        let col = self.cf_names.len() as u32;
        let name = format!("col{}", col);
        let _ = self
            .db
            .create_cf(&name, &col_config(config, &self.block_opts)?)
            .map_err(other_io_err)?;
